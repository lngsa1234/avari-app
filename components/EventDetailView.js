...
export default function EventDetailView({ currentUser, supabase: supabaseProp, onNavigate, meetupId, previousView, onMeetupChanged }) {
  ...
  async function loadEventDetails() {
    try {
      ...
      // Load pre-generated discussion topics from cache (non-blocking)
      loadSuggestedTopics(meetupData);
  ...
  async function loadSuggestedTopics(meetupData) {
    if (!meetupData) return;
    try {
      // Read pre-generated topics from cache (generated by daily cron 24h before event)
      const res = await fetch(`/api/agent/discussion-topics?meetupId=${meetupData.id}`);
      if (res.ok) {
        const data = await res.json();
        if (data.found && data.topics) {
