...
export default function MeetupsView({ currentUser, supabase, connections = [], meetups = [], userSignups = [], onNavigate, initialView = null, pastOnly = false, toast }) {
  ...
  const { data: pastMeetups = EMPTY, isLoading: pastLoading } = useSupabaseQuery(
    ...
    try {
      ...
      const [recapResult, coffeeResult, signupsResult, membershipResult] = await Promise.all([
        supabase
          .from('call_recaps')
          .select('id, channel_name, call_type, provider, started_at, ended_at, duration_seconds, participant_count, participant_ids, transcript_path, ai_summary, metrics, created_by, created_at, updated_at')
