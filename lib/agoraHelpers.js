  ...
  try {
    ...
    if (existingRoom) {
      ...
      return {
        ...
        link: `${window.location.origin}/call/meetup/${existingRoom.channel_name}`
      };
    }
    ...
    const link = `${window.location.origin}/call/meetup/${channelName}`;
...
/**
 * Get call recap data including room info and participants
 * @param {string} channelName - The channel name
 * @param {string} meetupId - Optional meetup ID to get signup participants
 * @returns {Promise<Object>} Recap data
 */
export async function getCallRecapData(channelName, meetupId = null) {
  try {
    ...
    // Get participants from meetup signups if meetupId provided
    let participants = [];
    if (meetupId) {
      // First get user IDs from signups
      ...
      if (signups && signups.length > 0) {
        // Then fetch profiles separately to avoid FK join issues
    ...
    // If no meetup participants, extract from messages
    ...
    console.error('Error getting call recap data:', error);
    return {
      room: null,
      participants: [],
      messages: [],
