...
export async function getAllProposals(supabase) {
  try {
    ...
    // Batch-fetch proposer profiles in a single query (dedup user ids first).
    ...
    const withProfiles = proposals.map(p => ({ ...p, proposer: byId[p.user_id] || null }));

    console.log(`✅ Loaded ${withProfiles.length} proposals`);
    return withProfiles;
  } catch (error) {
