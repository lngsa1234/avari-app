...
export async function requestToJoinGroup(supabase, groupId, userId) {
  // Check if already a member (use maybeSingle to avoid 406 when no rows)
  ...
  if (checkError) {
    return { success: false, error: checkError.message || 'Failed to check membership' };
  }
  ...
  // Insert as a pending join request (distinct from 'invited' which is creator-initiated)
  const { error } = await supabase
    .from('connection_group_members')
    .insert({
      group_id: groupId,
