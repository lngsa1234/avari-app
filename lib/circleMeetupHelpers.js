...
export async function getOrCreateCircleMeetups(circleId, circle, count = 4) {
  try {
    ...
    // Filter to future meetups for display
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const todayStr = today.toISOString().split('T')[0];

    const existingMeetups = (allExistingMeetups || []).filter(m => m.date >= todayStr);

    // If we have enough future meetups, return them
    ...
    if (meetupsToCreate.length > 0) {
      ...
      allFutureMeetups.sort((a, b) => new Date(a.date) - new Date(b.date));
