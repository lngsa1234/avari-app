...
import { parseLocalDate, isEventPast, SESSION_GRACE_MINUTES } from './dateUtils';

/**
 * Resolve display info for a circle's host.
 *
 * Tries multiple sources in order:
 *   1. `circle.creator` — dedicated creator profile (preferred, fetched
 *      independently of membership so it works even if the creator isn't
 *      in the accepted-members list)
 *   2. `circle.members.find(m => m.user_id === circle.creator_id).user` —
 *      fallback for shapes that only have a members list
 *
 * Returns a display object with sensible defaults. If neither source has a
 * usable name, returns `initial` = first letter of the circle name (e.g.
 * "T" for "Test2") rather than the generic "H" from the string "Host".
 *
 * This function fixes a bug where the home page "Circle to Join" card
 * showed "H / Host" instead of the actual creator's name when the creator
 * wasn't in `connection_group_members` with status='accepted' (possible
 * for legacy circles) or when the creator's profile was filtered out
 * from the in-fetch (e.g. private profile + RLS).
 *
 * @param {Object} circle
 * @returns {{name: string, initial: string, profile_picture: string|null, resolved: boolean}}
 */
export function resolveCircleHost(circle) {
  if (!circle) {
    return { name: 'Host', initial: 'H', profile_picture: null, resolved: false };
  }

  // Source 1: dedicated creator profile field (preferred)
  if (circle.creator?.name) {
    return {
      name: circle.creator.name,
      initial: circle.creator.name.trim().charAt(0).toUpperCase() || '?',
      profile_picture: circle.creator.profile_picture || null,
      resolved: true,
    };
  }

  // Source 2: find creator in the members list by user_id
  if (circle.creator_id && Array.isArray(circle.members)) {
    const creatorMember = circle.members.find(m => m.user_id === circle.creator_id);
    if (creatorMember?.user?.name) {
      return {
        name: creatorMember.user.name,
        initial: creatorMember.user.name.trim().charAt(0).toUpperCase() || '?',
        profile_picture: creatorMember.user.profile_picture || null,
        resolved: true,
      };
    }
  }

  // Fallback: unknown host. Use the circle name's initial so the avatar
  // shows a letter tied to the circle rather than "H" from "Host".
  const fallbackInitial = circle.name?.trim().charAt(0).toUpperCase() || '?';
  return {
    name: 'Host',
...
/**
 * Explicit column list for meetups queries in the circle flow.
 *
 * Replaces `select('*')` on hot-path queries that load meetup cards
 * on the circle detail page. Matches the UI-side explicit list already
 * used at components/CircleDetailView.js:294 for past meetups, plus
 * the fields this file's own filter/update logic reads internally
 * (timezone, circle_id) and the render fields used by
 * components/CircleDetailView.js around line 1272-1315 (description,
 * image_url).
 *
 * If you add a column the circle detail view needs to render, add it
 * here AND to the past-meetups query in CircleDetailView.js:294 so
 * both paths stay in sync.
 */
const CIRCLE_MEETUP_COLUMNS =
  'id, topic, description, date, time, duration, timezone, location, participant_limit, status, circle_id, created_by, image_url, agora_link, created_at, updated_at';
...
export async function getOrCreateCircleMeetups(circleId, circle, count = 4) {
  try {
    ...
    // For today's meetups, also check if the meeting time + grace has passed
    const activeFutureMeetups = (allExistingMeetups || []).filter(m => {
      if (m.date < todayStr) return false;
      if (m.status === 'cancelled' || m.status === 'completed') return false;
      if (m.date === todayStr && m.time && isEventPast(m.date, m.time, m.timezone, parseInt(m.duration || '60'), SESSION_GRACE_MINUTES)) return false;
      return true;
    });
