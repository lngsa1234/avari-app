...
/**
 * Explicit column list for coffee_chats list queries.
 *
 * Replaces `select('*')` on hot-path list queries (home page, coffee page,
 * pending/sent request tabs). Narrowing the select reduces egress per row
 * and stops new columns from automatically joining the wire format when
 * they're added later — forcing a conscious decision instead.
 *
 * If you add a column that the UI needs to render, add it here AND update
 * any consumers in components/MainApp.js, components/MeetupsView.js,
 * hooks/useHomeData.js, or app/(app)/home/page.js.
 */
const COFFEE_CHAT_LIST_COLUMNS =
  'id, requester_id, recipient_id, status, scheduled_time, duration, topic, notes, room_url, video_link, created_at, updated_at, completed_at';
...
export const requestCoffeeChat = async (supabase, { recipientId, scheduledTime, topic, notes }) => {
  try {
...
// Mark a coffee chat as abandoned (call never connected or no-show)
export const abandonCoffeeChat = async (supabase, chatId) => {
  const { error } = await supabase
    .from('coffee_chats')
...
export const getMyCoffeeChats = async (supabase) => {
  try {
    ...
    const { data: chats, error } = await supabase
      .from('coffee_chats')
      .select(COFFEE_CHAT_LIST_COLUMNS)
