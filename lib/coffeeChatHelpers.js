...
export const getPendingRequests = async (supabase) => {
  try {
    ...
    // Auto-decline stale requests whose scheduled_time has passed
    const now = new Date();
    const stale = requests.filter(r => r.scheduled_time && new Date(r.scheduled_time) < now);
    const active = requests.filter(r => !r.scheduled_time || new Date(r.scheduled_time) >= now);
