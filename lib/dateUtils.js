...
// Unified grace period: how long after scheduled end a meeting stays "upcoming"
export const SESSION_GRACE_MINUTES = 60

const TERMINAL_STATUSES = ['completed', 'cancelled', 'declined', 'abandoned']

/**
 * Determine if a coffee chat should appear in "upcoming" views.
 * Single source of truth — replaces 4 different inline filter calculations.
 *
 * @param {{ status: string, scheduled_time?: string }} chat
 * @returns {boolean}
 */
export function isCoffeeChatUpcoming(chat) {
  if (TERMINAL_STATUSES.includes(chat.status)) return false
  if (!chat.scheduled_time) return true
  const chatEnd = new Date(new Date(chat.scheduled_time).getTime() + 30 * 60000)
  const withGrace = new Date(chatEnd.getTime() + SESSION_GRACE_MINUTES * 60000)
  return new Date() <= withGrace
}
...
export function formatEventTime(dateStr, timeStr, eventTimezone, { showTimezone = true } = {}) {
  if (!timeStr) return '';
  ...
  if (eventTimezone && eventTimezone !== viewerTz) {
    ...
    try {
      ...
      return showTimezone ? `${formatted} ${getTimezoneLabel(viewerTz, eventUTC)}` : formatted;
    } catch {
      // Fall through to default formatting
    }
  }

  // Same timezone or no event timezone — format as-is
  ...
  return showTimezone ? `${timeFormatted} ${getTimezoneLabel(viewerTz)}` : timeFormatted;
}
