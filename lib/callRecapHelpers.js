...
/**
 * Fetch transcript + AI summary from Supabase Storage via server API.
 * Uses the server-side admin client to bypass storage RLS policies.
...
export async function getRecapTranscriptFromStorage(transcriptPath) {
  ...
  try {
    ...
    const response = await apiFetch(`/api/get-recap-transcript?path=${encodeURIComponent(transcriptPath)}`);
