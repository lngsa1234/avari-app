...
async function loadProvider(providerType) {
  switch (providerType) {
    case ProviderType.AGORA:
      if (!AgoraProvider) {
        const mod = await import('./AgoraProvider');
        AgoraProvider = mod.default;
      }
      return AgoraProvider;

    case ProviderType.LIVEKIT:
      if (!LiveKitProvider) {
        const mod = await import('./LiveKitProvider');
        LiveKitProvider = mod.default;
      }
    ...
    default:
      if (!WebRTCProvider) {
        const mod = await import('./WebRTCProvider');
        WebRTCProvider = mod.default;
      }
