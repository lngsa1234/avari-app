...
export function attachTrack(track, element, provider, trackType = 'video') {
  ...
  try {
    switch (provider) {
      case 'agora':
        if (trackType === 'audio' && track.getMediaStreamTrack) {
          // Play remote audio through a standard <audio> element so Chrome's
          // echo cancellation has a reference signal.  Agora's own play() uses
          // Web Audio API internally which bypasses the browser's AEC.
          const rawAudioTrack = track.getMediaStreamTrack();
          if (rawAudioTrack && element.tagName === 'AUDIO') {
            element.srcObject = new MediaStream([rawAudioTrack]);
            element.play?.().catch(() => {});
          } else if (track.play) {
            // Fallback
            track.play();
          ...
          // Video tracks: use Agora's play() for rendering into a div
      ...
      case 'webrtc': {
        ...
        if (element.tagName === 'DIV') {
          ...
          if (!mediaEl) {
            ...
            if (tag === 'video') {
              // Mute the video element — audio is played via separate <audio> element.
              // This also allows autoplay without user gesture on most browsers.
              mediaEl.muted = true;
...
export function detachTrack(track, element, provider) {
  ...
  try {
    switch (provider) {
      case 'agora':
        // Clear srcObject if we attached audio via standard <audio> element
        if (element.tagName === 'AUDIO' && element.srcObject) {
          element.srcObject = null;
        }
        // Note: don't call track.stop() — it stops the track completely
        break;
