...
export async function getMyConnectionGroups() {
  try {
    ...
    const [
      ...
      // 4. Get upcoming meetups for all circles
      supabase
        .from('meetups')
        .select('id, topic, date, time, location, circle_id')
    ...
    // Get next meetup per circle (first one since ordered by date asc)
    const nextMeetupByCircle = {};
    (meetupsResult.data || []).forEach(meetup => {
      if (!nextMeetupByCircle[meetup.circle_id]) {
