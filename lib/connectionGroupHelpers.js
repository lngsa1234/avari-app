 ...
 */
export async function createConnectionGroup({ name, description, invitedUserIds }) {
  try {
 ...
 * @returns {Promise<Array>}
 */
export async function getMyConnectionGroups(userId) {
  try {
 ...
 * Uses meetupId for unique per-session channel names (isolates transcripts/recaps)
 * @param {string} groupId - The connection group ID
 * @param {string} [meetupId] - The meetup ID for session-specific room naming
 * @returns {Promise<{roomId: string, channelName: string, link: string}>}
 */
export async function createConnectionGroupRoom(groupId, meetupId) {
  try {
    console.log('🎥 Creating connection group room for group:', groupId, 'meetup:', meetupId);

    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('Not authenticated');

    // Use meetupId for unique per-session channels; fall back to groupId for legacy
    const channelName = meetupId
      ? `connection-group-${meetupId}`
    ...
    if (meetupId) {
      existingQuery = existingQuery.eq('channel_name', channelName);
    } else {
