 ...
 * @param {Array<string>} params.invitedUserIds - Array of user IDs to invite (2-3 people)
...
export async function createConnectionGroup({ name, invitedUserIds }) {
  try {
    ...
    // Validate: 3-4 people total (creator + 2-3 invitees)
    if (invitedUserIds.length < 2 || invitedUserIds.length > 3) {
      throw new Error('Groups must have 3-4 people total (including you)');
    }
...
export async function getMyConnectionGroups() {
  try {
    console.log('📋 Loading connection groups...');

    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('Not authenticated');

    // Get groups where user is creator or accepted member
    ...
    // Extract group IDs
    const groupIds = memberGroups.map(mg => mg.connection_groups.id);
    ...
    // Get full group details with all members
    ...
    // Manually fetch creator and member details
    for (const group of groups) {
      ...
      if (members) {
        // Fetch profiles for each member (including last_active for activity tracking)
        for (const member of members) {
          const { data: profile, error: profileError } = await supabase
            .from('profiles')
            .select('id, name, career, last_active')
    ...
    console.log(`✅ Loaded ${groups.length} connection groups`);
...
export async function getPendingGroupInvites() {
  try {
    ...
    // Manually fetch group and creator details for each invite
  ...
  try {
    ...
    if (existing) {
      ...
      return {
        ...
        link: `${window.location.origin}/connection-group-call/${existing.channel_name}`
      };
    }
    ...
    const link = `${window.location.origin}/connection-group-call/${channelName}`;
