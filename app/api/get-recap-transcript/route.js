 ...
 *
 * Note: there is intentionally no outer try/catch around this handler.
 * Wrapping the whole thing in try/catch swallowed Next.js's internal
 * Dynamic server usage signal during static generation, causing the
 * route to be pre-rendered as a static empty-body response at build
 * time. Real runtime failures (auth, DB, network) are now allowed to
 * bubble up to produce visible 500s instead of silent empty 200s.
 */
export async function GET(request) {
  ...
  // Narrow try/catch: only the two operations below can actually throw
  // (blob .text() can fail, JSON.parse can throw on malformed payload).
  try {
    ...
    console.error('[GetRecapTranscript] Malformed transcript JSON at', transcriptPath, parseError);
    return NextResponse.json({ transcript: [], aiSummary: null });
  }
}
