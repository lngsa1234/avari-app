-- Add video provider performance metrics to admin analytics RPC
-- Run this migration in the Supabase SQL editor AFTER create-admin-analytics-rpc.sql
-- and database-migration-call-recaps.sql (which creates provider_metrics table)
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- ============================================================
    -- VIDEO PROVIDER PERFORMANCE METRICS (new)
    -- ============================================================

    -- Provider performance summary (from provider_metrics table)
    'provider_metrics_summary', (
      ...
      FROM (
        SELECT
          ...
          ROUND(AVG(avg_bitrate_kbps), 0) AS avg_bitrate,
          ROUND(AVG(participant_count)::numeric, 1) AS avg_participants,
          COUNT(CASE WHEN connection_quality = 'excellent' THEN 1 END) AS excellent_count,
          COUNT(CASE WHEN connection_quality = 'good' THEN 1 END) AS good_count,
          COUNT(CASE WHEN connection_quality = 'fair' THEN 1 END) AS fair_count,
          COUNT(CASE WHEN connection_quality = 'poor' THEN 1 END) AS poor_count
        FROM provider_metrics
        WHERE recorded_at > now_ts - (days_back || ' days')::INTERVAL
        GROUP BY provider, call_type
        ORDER BY provider, call_type
      ) t
    ),

    -- Calls by provider (from call_recaps)
    'calls_by_provider', (
      SELECT COALESCE(jsonb_agg(row_to_json(t)::jsonb), '[]'::jsonb)
      FROM (
        SELECT
    ...
    -- Calls by week and provider (for stacked bar chart)
    'calls_by_week_provider', (
      SELECT COALESCE(jsonb_agg(row_to_json(t)::jsonb ORDER BY t.week), '[]'::jsonb)
      FROM (
        SELECT
          DATE_TRUNC('week', created_at)::DATE AS week,
          COUNT(*) FILTER (WHERE provider = 'webrtc') AS webrtc,
          COUNT(*) FILTER (WHERE provider = 'livekit') AS livekit,
          COUNT(*) FILTER (WHERE provider = 'agora') AS agora,
          COUNT(*) AS total
        FROM call_recaps
        WHERE created_at > now_ts - (days_back || ' days')::INTERVAL
        GROUP BY DATE_TRUNC('week', created_at)
